        ...
        function VirtualChatFlow({ order, nodeStore, listRef, prependRef, useSession, selectedCallId, cwd, openFile, inspectCall, forkAt, loadImage, fileMentions, renderSlot, t, virtualApiRef, autoFollowRef }) {
			...
			(0, react.useLayoutEffect)(() => {
					...
					if (index > 0) {
						...
						if (request?.key !== null && request?.key !== void 0) {
							...
							const correct = () => {
								...
								if (row !== null) {
									el.scrollTop += flowTop(row, el) - request.top;
									scrollTopRef.current = el.scrollTop;
				...
				if (!prepended && anchor !== null) {
					...
					if (index >= 0) {
						...
						el.scrollTop = anchor.scrollTop + nextOffset - anchor.offset;
						scrollTopRef.current = el.scrollTop;
					}
				}
				previousFlowRef.current = { firstKey: order[0] ?? null, firstOffset: geometry.offsets[0] ?? 0 };
				if (autoFollowRef?.current === true && measureTick > 0) {
					if (followSettledRef.current === geometry.total) {
						el.scrollTop = el.scrollHeight;
						scrollTopRef.current = el.scrollTop;
						autoFollowRef.current = false;
					} else followSettledRef.current = geometry.total;
				}
			...
			(0, react.useEffect)(() => {
				...
				const observer = new ResizeObserver((entries) => {
					...
					if (changed && measureFrameRef.current === 0) {
		...
		function OutlinePanel({ useSession, listRef, loadOlder, virtualApiRef }) {
			...
			const exactKey = (seq) => {
				for (const node of nodes.values()) {
					if (node.kind === "assistant-step" && node.visibility !== "hidden" && node.anchorSeq === seq) return node.key;
				}
				return null;
			};
			const nearestKey = (seq) => {
				...
				for (const node of nodes.values()) {
					...
					if (node.anchorSeq <= seq && (best === null || node.anchorSeq > best.anchorSeq)) best = node;
				}
			...
			const items = (0, react.useMemo)(() => {
				const list = outline ?? [];
				return list.length ? list.map((item, index) => ({ id: String(item.seq) + ":" + index, label: " ".repeat(Math.max(0, (item.level ?? 1) - 1)) + item.title })) : [{ id: "empty", label: "本会话暂无标题", disabled: true }];
			}, [outline]);
			const onSelect = (id) => { const seq = Number(id.split(":")[0]); if (!Number.isFinite(seq)) return; pending.current = { seq, paged: false }; setOpen(false); tryJump(); };
			return react_jsx_runtime.jsx("div", { style: { position: "fixed", right: 24, bottom: "calc(var(--dsh-composer-height, 152px) + 64px)", zIndex: 20, boxShadow: "var(--dsw-shadow-lv2)", borderRadius: 16 }, children: react_jsx_runtime.jsx(_deepseek_ai_dsh_client_ui_primitives.Menu, { open, anchor: react_jsx_runtime.jsx(_deepseek_ai_dsh_client_ui_primitives.Button, { variant: "toolbar", size: "sm", icon: react_jsx_runtime.jsx(_deepseek_ai_dsh_client_ui_primitives.IconListPenOutline16, {}), onClick: () => setOpen((value) => !value), children: "大纲 · " + (outline?.length ?? 0) }), items, onSelect, onClose: () => setOpen(false), portal: true, side: "top", align: "end" }) });
		}
			...
			*  scrolls the rest of the way to the floor). */
			...
			(0, react.useLayoutEffect)(() => {
				...
				if (openState === "open" && !openedRef.current) {
					...
					if (saved === null) {
						requestAnimationFrame(() => {
							const row = pagingAnchor(listRef.current ?? local, scrollerOf(listRef.current ?? local));
							if (row === null) toBottom(el, true);
						});
					} else {
