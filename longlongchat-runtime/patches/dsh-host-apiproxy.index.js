...
/** Extract a conversation outline grouped by user prompts, with one merged assistant summary per section. */
function outlineOf(events) {
	const outline = [];
	let section = null;
	const flush = () => {
		if (section !== null && section.answer !== null) outline.push({ seq: section.answerSeq, level: 2, title: section.answer });
		section = null;
	};
	for (const event of events) {
		if (event.type === "user/message" && event.data?.source?.kind === "user") {
			const title = compactOutlineTitle(messageText(event), 160);
			if (title === null) continue;
			const normalized = normalizeOutlineTitle(title);
			if (section !== null && normalized !== "" && (normalized === section.normalized || normalized.startsWith(section.normalized) || section.normalized.startsWith(normalized) || normalized.length >= 8 && (section.normalized.includes(normalized) || normalized.includes(section.normalized)))) {
				section.item = { ...section.item, seq: event.seq, title };
				section.normalized = normalized;
				continue;
			}
			flush();
			section = { item: { seq: event.seq, level: 1, title }, normalized, answer: null, answerSeq: null };
			outline.push(section.item);
		} else if (event.type === "assistant/message" && section !== null) {
			const answer = compactOutlineTitle(assistantParagraphs(messageText(event)).join(" "), 160);
			if (answer === null) continue;
			section.answer = answer;
			section.answerSeq = event.seq;
		}
	}
	flush();
	return outline;
