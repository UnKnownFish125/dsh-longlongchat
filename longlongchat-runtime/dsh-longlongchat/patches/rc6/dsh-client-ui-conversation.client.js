window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var ConversationController = class extends _deepseek_ai_cordis.Service {
			...
			*/
			...
			async loadOlder(options = {}) {
				await this.scopedSession("loadOlder").loadOlder(options);
			}
		...
		*/
		...
		/** Scrollport content coordinate of a row's viewport position. */
		function flowOffsetAt(scrollport, anchorTop) {
			if (typeof getComputedStyle !== "function") return Math.max(0, scrollport.scrollTop + (anchorTop || 0));
			const paddingTop = Number.parseFloat(getComputedStyle(scrollport).paddingTop) || 0;
			return Math.max(0, scrollport.scrollTop + (anchorTop || 0) - paddingTop);
		}
		function flowOffset(row, scrollport) {
			return flowOffsetAt(scrollport, flowTop(row, scrollport));
		}
		...
		*/
		function estimateChatRow(node) {
			if (node?.kind === "user") return 86;
			if (node?.kind === "assistant-step") return 180;
			if (node?.kind === "tool-call" || node?.kind === "tool-result") return 150;
			return 120;
		}
		/** Bounded DOM chat flow used by the isolated LongLongChat test machine. */
        function VirtualChatFlow({ order, nodeStore, listRef, prependRef, useSession, selectedCallId, cwd, openFile, inspectCall, forkAt, loadImage, fileMentions, renderSlot, t, virtualApiRef, autoFollowRef }) {
			const [scrollTick, setScrollTick] = (0, react.useState)(0);
			const [measureTick, setMeasureTick] = (0, react.useState)(0);
			const measuredRef = (0, react.useRef)(new Map());
			const visibleKeysRef = (0, react.useRef)([]);
			const scrollFrameRef = (0, react.useRef)(0);
			const measureFrameRef = (0, react.useRef)(0);
			const measureAnchorRef = (0, react.useRef)(null);
			const scrollTopRef = (0, react.useRef)(0);
			const previousFlowRef = (0, react.useRef)(null);
			const restoringRef = (0, react.useRef)(false);
			const pendingCorrectionRef = (0, react.useRef)(null);
			const pendingMeasureRef = (0, react.useRef)(false);
			(0, react.useEffect)(() => {
			...
			const followSettledRef = (0, react.useRef)(null);
			const geometry = (0, react.useMemo)(() => {
				...
				for (const key of order) {
					...
					const height = measuredRef.current.get(key) ?? estimateChatRow(node);
					cursor += height + 16;
				}
				return { offsets, total: Math.max(0, cursor - (order.length ? 16 : 0)) };
			}, [order, nodeStore, measureTick]);
			const scrollToKey = (0, react.useCallback)((key) => {
				const local = listRef.current;
				if (local === null) return false;
				const el = scrollerOf(local);
				const index = order.indexOf(key);
				if (index < 0) return false;
				const targetOffset = geometry.offsets[index] ?? 0;
				const rowHeight = measuredRef.current.get(key) ?? estimateChatRow(nodeStore.get(key));
				el.scrollTop = Math.max(0, targetOffset - Math.max(16, (el.clientHeight - rowHeight) / 2));
				scrollTopRef.current = el.scrollTop;
				if (autoFollowRef !== void 0) autoFollowRef.current = false;
				return true;
			}, [order, nodeStore, geometry]);
			if (virtualApiRef !== void 0) virtualApiRef.current = { scrollToKey };
			const viewport = listRef.current ? scrollerOf(listRef.current) : null;
			const top = viewport?.scrollTop ?? 0;
			const height = viewport?.clientHeight ?? 800;
			const overscan = Math.max(900, height * 1.5);
			let start = 0;
			while (start < order.length - 1 && geometry.offsets[start + 1] < top - overscan) start += 1;
			let end = start;
			while (end < order.length && geometry.offsets[end] < top + height + overscan) end += 1;
			const visibleKeys = order.slice(start, end);
			...
			(0, react.useLayoutEffect)(() => {
				...
				const prepended = previous !== null && previous.firstKey !== null && previous.firstKey !== order[0];
				if (prepended) {
					const index = order.indexOf(previous.firstKey);
					if (index > 0) {
						const nextOffset = geometry.offsets[index] ?? 0;
						const request = prependRef.current;
						const baseTop = request?.scrollTop ?? el.scrollTop;
						el.scrollTop = baseTop + nextOffset - previous.firstOffset;
						scrollTopRef.current = el.scrollTop;
						restoringRef.current = true;
						pendingMeasureRef.current = true;
						measureAnchorRef.current = null;
						if (request?.key !== null && request?.key !== void 0) {
							let attempts = 0;
							pendingCorrectionRef.current = () => {
								const row = anchorElement(local, request.key);
								if (row !== null) {
									const base = request.scrollTop ?? el.scrollTop;
									const expected = request.scrollOffset ?? (base + (request.top ?? 0));
									const actual = flowOffset(row, el);
									el.scrollTop += expected - actual;
									scrollTopRef.current = el.scrollTop;
									...
									if (pendingMeasureRef.current) {
										...
										requestAnimationFrame(() => setMeasureTick((value) => value + 1));
									}
									return;
								}
								if (++attempts < 12) requestAnimationFrame(() => { pendingCorrectionRef.current?.(); });
								else {
									prependRef.current = null;
									pendingCorrectionRef.current = null;
									restoringRef.current = false;
							...
							requestAnimationFrame(() => { pendingCorrectionRef.current?.(); });
						} else {
							prependRef.current = null;
							restoringRef.current = false;
							pendingMeasureRef.current = false;
				...
				const anchor = autoFollowRef?.current === true ? null : measureAnchorRef.current;
				if (prepended || autoFollowRef?.current === true) measureAnchorRef.current = null;
				if (!prepended && anchor !== null) {
					measureAnchorRef.current = null;
					const index = order.indexOf(anchor.key);
					if (index >= 0) {
						const nextOffset = geometry.offsets[index] ?? anchor.offset;
						el.scrollTop += nextOffset - anchor.offset;
						scrollTopRef.current = el.scrollTop;
					}
				}
				previousFlowRef.current = { firstKey: order[0] ?? null, firstOffset: geometry.offsets[0] ?? 0 };
				if (autoFollowRef?.current === true && !restoringRef.current && followSettledRef.current !== el.scrollHeight) {
					el.scrollTop = el.scrollHeight;
					followSettledRef.current = el.scrollHeight;
					scrollTopRef.current = el.scrollTop;
				}
			}, [geometry.total, order, listRef]);
			(0, react.useEffect)(() => {
				const local = listRef.current;
				if (local === null || typeof ResizeObserver === "undefined") return;
				const el = scrollerOf(local);
				const observer = new ResizeObserver((entries) => {
					let changed = false;
					let anchorIndex = start;
					while (anchorIndex < order.length - 1) {
						const key = order[anchorIndex];
						const rowHeight = measuredRef.current.get(key) ?? estimateChatRow(nodeStore.get(key));
						if (geometry.offsets[anchorIndex] + rowHeight >= el.scrollTop) break;
						anchorIndex += 1;
					}
					const candidate = order[anchorIndex] === void 0 ? null : { key: order[anchorIndex], offset: geometry.offsets[anchorIndex], scrollTop: el.scrollTop };
					for (const entry of entries) {
						...
						if (next > 0 && (previous === void 0 || Math.abs(previous - next) > 1)) { measuredRef.current.set(key, next); changed = true; }
					}
					if (changed && measureFrameRef.current === 0 && !restoringRef.current && !pendingMeasureRef.current) {
						if (measureAnchorRef.current === null && autoFollowRef?.current !== true) measureAnchorRef.current = candidate;
				...
				for (const key of visibleKeys) {
					const row = local.querySelector('[data-chat-anchor-key="' + key.replaceAll('"', '\\\"') + '"]');
					if (row) observer.observe(row);
				}
				return () => {
					observer.disconnect();
					if (measureFrameRef.current !== 0) cancelAnimationFrame(measureFrameRef.current);
					measureFrameRef.current = 0;
				};
			}, [visibleKeys.join("|"), listRef]);
			return react_jsx_runtime.jsx("div", { style: { position: "relative", width: "100%", height: geometry.total }, "data-chat-virtual-flow": "", children: visibleKeys.map((nodeKey, offset) => react_jsx_runtime.jsx("div", { style: { position: "absolute", left: 0, right: 0, top: geometry.offsets[start + offset], minWidth: 0 }, children: react_jsx_runtime.jsx(ChatNodeSeat, { nodeKey, useSession, selectedCallId, cwd, openFile, inspectCall, forkAt, loadImage, fileMentions, renderSlot, t }) }, nodeKey)) });
		}
		function OutlinePanel({ useSession, listRef, loadOlder, virtualApiRef }) {
			...
			const firstSeq = firstKey === void 0 ? null : nodes.get(firstKey)?.anchorSeq ?? null;
			stateRef.current = { order, nodes, hasMore, loadingOlder, openState, firstSeq };
			const exactKey = (seq) => {
				let step = null;
				for (const node of nodes.values()) {
					if (node.visibility === "hidden" || node.anchorSeq !== seq) continue;
					if (node.kind === "user") return node.key;
					if (node.kind === "assistant-step" && step === null) step = node.key;
				}
				return step;
			};
			const nearestKey = (seq) => {
				let best = null;
				let bestStep = null;
				for (const node of nodes.values()) {
					if (node.visibility === "hidden" || node.anchorSeq > seq) continue;
					if (node.kind === "user" && (best === null || node.anchorSeq > best.anchorSeq)) best = node;
					else if (node.kind === "assistant-step" && (bestStep === null || node.anchorSeq > bestStep.anchorSeq)) bestStep = node;
				}
				return best !== null ? best.key : bestStep === null ? null : bestStep.key;
			};
			const tryJump = (0, react.useCallback)(() => {
				const target = pending.current;
				if (target === null) return;
				const state = stateRef.current;
				let key = exactKey(target.seq);
				if (key === null && state.hasMore && state.firstSeq !== null && target.seq < state.firstSeq && state.openState === "open" && !state.loadingOlder) {
					target.paged = true;
				...
				if (key !== null) {
					const local = listRef.current;
					const el = local ? scrollerOf(local) : null;
					const jumped = virtualApiRef?.current?.scrollToKey(key) ?? false;
					if (!jumped && el !== null) {
						requestAnimationFrame(() => { const anchor = el.querySelector('[data-chat-anchor-key="' + key + '"]'); anchor?.scrollIntoView({ behavior: "smooth", block: "start" }); });
					}
					pending.current = null;
				}
			}, [order, nodes, listRef, loadOlder]);
			(0, react.useEffect)(() => { if (!loadingOlder) tryJump(); }, [loadingOlder, tryJump, order.length]);
			const items = (0, react.useMemo)(() => {
				const list = outline ?? [];
				return list.length ? list.map((item, index) => ({ id: String(item.seq) + ":" + index, level: Math.max(1, item.level ?? 1), title: item.title })) : [{ id: "empty", level: 1, title: "本会话暂无标题", disabled: true }];
			}, [outline]);
			const onSelect = (id) => { const seq = Number(id.split(":")[0]); if (!Number.isFinite(seq)) return; pending.current = { seq, paged: false }; tryJump(); };
			const onDragStart = (e) => {
				const rect = e.currentTarget.getBoundingClientRect();
				dragRef.current = { x: e.clientX, y: e.clientY, left: rect.left, top: rect.top };
				const move = (ev) => { const d = dragRef.current; if (!d) return; setPos({ left: Math.max(0, d.left + ev.clientX - d.x), top: Math.max(0, d.top + ev.clientY - d.y) }); };
			...
			const panelStyle = { position: "fixed", zIndex: 30, width: 288, borderRadius: 14, overflow: "hidden", boxShadow: "var(--dsw-shadow-lv2)", background: "var(--dsw-surface, #ffffff)", color: "var(--dsw-text, #1f2328)", border: "1px solid var(--dsw-border, rgba(0,0,0,0.08))", ...(pos ? { left: pos.left, top: pos.top } : { right: 24, bottom: "calc(var(--dsh-composer-height, 152px) + 64px)" }) };
			return react_jsx_runtime.jsx("div", { style: panelStyle, children: [
				react_jsx_runtime.jsx("div", { onPointerDown: onDragStart, style: { display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", cursor: "grab", background: "var(--dsw-surface-2, rgba(0,0,0,0.03))", userSelect: "none", touchAction: "none" }, children: [
					react_jsx_runtime.jsx(_deepseek_ai_dsh_client_ui_primitives.IconListPenOutline16, {}),
					react_jsx_runtime.jsx("span", { style: { flex: 1, fontSize: 12, fontWeight: 600 }, children: "大纲 · " + (outline?.length ?? 0) }),
					react_jsx_runtime.jsx("button", { type: "button", onPointerDown: (e) => e.stopPropagation(), onClick: () => setCollapsed((v) => !v), style: { border: "none", background: "transparent", cursor: "pointer", fontSize: 16, lineHeight: 1, margin: 0, padding: "2px 4px", color: "inherit" }, children: collapsed ? "+" : "−" })
				] }),
				collapsed ? null : react_jsx_runtime.jsx("div", { style: { maxHeight: "min(48vh, 360px)", overflowY: "auto", padding: "4px" }, children: items.map((item) => react_jsx_runtime.jsx("div", { role: "button", tabIndex: 0, onClick: () => onSelect(item.id), onKeyDown: (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); onSelect(item.id); } }, style: { padding: "6px 10px", paddingLeft: 10 + Math.min(3, (item.level ?? 1) - 1) * 14, fontSize: item.level <= 1 ? 13.5 : 12.5, lineHeight: 1.35, borderRadius: 8, cursor: item.disabled ? "default" : "pointer", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontWeight: item.level <= 1 ? 600 : 400, color: item.disabled ? "var(--dsw-muted, #999)" : item.level <= 1 ? "inherit" : "var(--dsw-muted, #777)" }, children: item.level > 1 ? "· " + item.title : item.title }, item.id)) })
			] });
		}
		function ChatView({ useSession, useSessions, useStore, renderSlot, sessionId, openFile, loadOlder, loadImage, inspectCall, chatScroll, forkAt, fileMentions, t }) {
			...
			const readerActiveRef = (0, react.useRef)(false);
			...
			* while the request is pending and restored after the prepend lands. */
			...
			const virtualApiRef = (0, react.useRef)(null);
			const autoFollowRef = (0, react.useRef)(false);
			...
			*  scrolls the rest of the way to the floor). */
			...
			const toBottom = (el, force = false) => {
				if (readerActiveRef.current && !force) return;
			...
			(0, react.useLayoutEffect)(() => {
				...
				lastSteeringIdRef.current = lastSteeringId;
				followSigRef.current = followSig;
				if (firstSeq === firstSeqRef.current && (appendedUser || appendedSteering || tipMoved && atBottomRef.current && !loadingOlder && anchorRef.current === null)) toBottom(el);
			});
			...
			onScrollRef.current = () => {
				...
				if (movedByReader && isAtBottom) { readerActiveRef.current = false; autoFollowRef.current = true; }
				else if (movedByReader && !isAtBottom) { readerActiveRef.current = true; autoFollowRef.current = false; }
				if (!movedByReader && isAtBottom && !loadingOlder) {
					toBottom(el);
					return;
				}
				atBottomRef.current = isAtBottom;
				...
				if (isAtBottom) anchorRef.current = null;
					...
					scrollHeight: anchorRef.current.scrollHeight
				};
				if (isAtBottom) chatScroll.save(null);
				else if (position !== null) chatScroll.save(position);
				if (el.scrollTop < Math.max(240, el.clientHeight * 1.5) && hasMore && !loadingOlder && openState === "open") loadOlderAnchored();
				observedTopRef.current = el.scrollTop;
			};
			(0, react.useEffect)(() => {
				const local = listRef.current;
			...
			followRef.current = () => {
				const local = listRef.current;
				if (local !== null && atBottomRef.current && !loadingOlder) {
					const el = scrollerOf(local);
					el.scrollTop = el.scrollHeight;
					observedTopRef.current = el.scrollTop;
					chatScroll.save(null);
			...
			(0, react.useEffect)(() => {
				...
				// Virtual flow total height changes on prepend; observing the column would
				// misclassify that as tail growth and snap the reader to the bottom.
			...
			const loadOlderAnchored = () => {
				...
				if (local !== null) {
					...
					anchorRef.current = {
						key: row?.dataset.chatAnchorKey ?? null,
						top: row === null ? 0 : flowTop(row, el),
						scrollTop: el.scrollTop,
						scrollHeight: el.scrollHeight,
						scrollOffset: row === null ? el.scrollTop : flowOffset(row, el)
					};
				}
				loadOlder();
			};
				...
				children: [(0, react_jsx_runtime.jsxs)("div", {
					...
					children: [(0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)(VirtualChatFlow, { order, nodeStore, listRef, prependRef: anchorRef, useSession, selectedCallId, cwd, openFile, inspectCall, forkAt, loadImage, fileMentions, renderSlot, t, virtualApiRef, autoFollowRef }),
						...
						children: (0, react_jsx_runtime.jsx)("button", {
							...
							onClick: () => {
								...
								if (local !== null) { readerActiveRef.current = false; toBottom(scrollerOf(local), true); }
							},
							children: (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconChevronDownOutline14, {})
						})
					})]
				}), (0, react_jsx_runtime.jsx)(OutlinePanel, { useSession, listRef, loadOlder, virtualApiRef })]
			});
		}
		//#endregion
