...
function compactOutlineTitle(text, max = 44) {
	...
	for (const sentence of sentences) {
		title += sentence.trim();
		if (title.length >= 24) break;
	}
	title = title.replace(/\s+/g, " ").trim();
	if (!title) title = clean.slice(0, 44);
...
/** Assistant live text without tool/thinking blocks. */
function assistantParagraphs(text) {
	const paragraphs = [];
	let inFence = false;
...
function normalizeOutlineTitle(text) {
	return (text || "").replace(/[^\p{L}\p{N}]+/gu, " ").trim().toLowerCase();
}
/** Parse markdown heading lines into {level,title} items (levels clamped to 1..4). */
function headingItems(text) {
	const out = [];
	for (const raw of (text || "").split("\n")) {
		...
		if (title) out.push({ level: Math.min(4, Math.max(1, m[1].length)), title });
	}
	return out;
}
/** Prefer real markdown headings; fall back to a single compacted line. */
function titleFor(text, fallbackLevel) {
	const heads = headingItems(text);
	if (heads.length) return heads;
	const compact = compactOutlineTitle(text, 160);
	return compact === null ? [] : [{ level: fallbackLevel, title: compact }];
}
/** Extract a conversation outline grouped into task sections: each section is
* one task — the user/assistant turns that work on the same thing. Consecutive
* turns merge into the open section unless an explicit task boundary appears:
* a user message that opens with a level-1 markdown heading, or an assistant
* reply that opens with a level-1 heading. Section seq points at the section's
* first user message so outline jumps land on user content. */
function outlineOf(events) {
	const outline = [];
	let section = null;
	const flush = () => {
		if (section === null) return;
		outline.push({ seq: section.itemSeq, level: 1, title: section.itemTitle });
		for (const entry of section.children) outline.push(entry);
		section = null;
	};
	for (const event of events) {
		if (event.type === "user/message" && event.data?.source?.kind === "user") {
			const text = messageText(event);
			const items = titleFor(text, 1);
			if (items.length === 0) continue;
			const root = items[0];
			const isH1 = headingItems(text).some((item) => item.level === 1);
			if (section !== null && !isH1) {
				// Same task merges into the open section: consecutive turns work
				// on one thing, so they form a single section whose jump point
				// stays the section's first user message. Real heading sub-titles
				// (level 2+) become children; compressed fallbacks are kept only
				// when they carry a heading.
				const sub = headingItems(text).length > 0 ? items : items.slice(1);
				for (const item of sub) section.children.push({ seq: event.seq, level: Math.max(2, Math.min(4, item.level)), title: item.title });
				continue;
			}
			flush();
			section = {
				...
				itemSeq: event.seq,
				normalized: normalizeOutlineTitle(root.title),
				children: items.slice(1).map((item) => ({ seq: event.seq, level: Math.max(2, Math.min(4, item.level)), title: item.title })),
				lastKey: null
			};
		} else if (event.type === "assistant/message" && section !== null) {
			const text = messageText(event);
			const heads = headingItems(text);
			if (heads.length > 0 && heads[0].level === 1) {
				// Assistant opened an explicit new task with an H1 heading.
				flush();
				section = {
					itemTitle: heads[0].title,
					itemSeq: event.seq,
					normalized: normalizeOutlineTitle(heads[0].title),
					children: heads.slice(1).map((item) => ({ seq: event.seq, level: Math.max(2, Math.min(4, item.level)), title: item.title })),
					lastKey: null
				};
				continue;
			}
			if (section.children.length >= 24) continue;
			// Prefer real markdown headings (kept at their own depth) so the
			// section keeps titled children; only compress when no heading exists.
			if (heads.length > 0) {
				for (const item of heads) {
					const key = item.level + ":" + item.title;
					if (section.lastKey === key) continue;
					section.lastKey = key;
					section.children.push({ seq: event.seq, level: Math.max(2, Math.min(4, item.level)), title: item.title });
				}
			} else {
				const items = titleFor(assistantParagraphs(text).join(" "), 2);
	...
	*/
	function historyCutOf(source, includeProjections) {
		if (source.kind === "detached") {
			...
			return {
				events: source.events,
				outline: outlineOf(source.events),
