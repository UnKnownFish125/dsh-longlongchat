...
export function apply(ctx, config = {}) {
  ...
  function pickCheapModel(llm) {
    const configured = String(config.extraction_model || '')
    if (configured.includes('/')) {
      const [provider, ...modelParts] = configured.split('/')
      const model = modelParts.join('/')
      if (provider && model) return { provider, model }
    }
    if (configured) return { provider: 'deepseek-official', model: configured }
    try {
      const models = llm.listModels ? llm.listModels('deepseek-official') : null
      if (models !== null && typeof models.then === 'function') return models.then(pickFromList).catch(() => null)
  ...
  async function refreshMemoryCache(sessionId, query) {
    ...
    const refresh = (async () => {
      ...
      state.injectCount += 1
      console.log('[longlongchat] session memory cache updated (total ' + state.injectCount + ')')
      return true
  ...
  ctx.on('system-prompt/assemble', async (assembly, context, next) => {
    ...
    try {
      ...
      console.error('[longlongchat] prompt cache refresh failed', String(e))
    }
    const text = memoryCache.get(sessionId) || ''
    return {
      ...assembled,
      sections: assembled.sections.map((section) => section.name === 'longlongchat' ? { ...section, text: text } : section),
    }
  })
